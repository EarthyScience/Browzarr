...
import { ZarrItem, ZarrMetadata, ZarrTitleDescription } from "./Interfaces";
...
export async function GetZarrMetadata(
	...
	for (const item of contents) {
		if (item.path && item.path.length > 1 && item.kind === "array") {
			const array = await zarr.open(group.resolve(item.path.substring(1)), {
				kind: "array",
			});
	...
	const description =
		"attrs" in group
			? "description" in group.attrs
				...
				: useGlobalStore.getState().initStore
			: useGlobalStore.getState().initStore;
	const title =
		"attrs" in group
