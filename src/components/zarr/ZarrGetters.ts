...
export async function GetZarrArray(variable: string){
    ...
    const typedArray = new Float16Array(new SharedArrayBuffer(totalElements * Float16Array.BYTES_PER_ELEMENT)); 
...
export async function GetZarrArrayWorkers(variable: string){
    const {idx4D, initStore, setProgress, setStrides, setStatus} = useGlobalStore.getState();
	const {compress, xSlice, ySlice, zSlice, currentStore, coarsen, kernelSize, kernelDepth, setCurrentChunks, setArraySize} = useZarrStore.getState()
	const {cache} = useCacheStore.getState();
    ...
    const sharedBuffer = new SharedArrayBuffer(totalElements * Float16Array.BYTES_PER_ELEMENT); 
    ...
    const scalingFactors:number[] = [] // A full list of scalingFactors to decide if need to do a final correction pass
        ...
        for (let y= yDim.start ; y < yDim.end ; y++){
            for (let x= xDim.start ; x < xDim.end ; x++){
                ...
                const isCacheValid = cachedChunk && 
                                    ...
                                    cachedChunk.kernel.kernelDepth === (coarsen ? kernelSize : undefined) ;
                if (isCacheValid){
                    const compressed = cachedChunk.compressed;
                    scalingFactors.push(cachedChunk.scalingFactor)
                    cachedTasks.push({
                        chunkData: cachedChunk.data,
                        chunkShape: cachedChunk.shape,
                        chunkStride: cachedChunk.stride,
                        chunkCoord: [z, y, x],
                        compressed
                    })
                }
    ...
    const cacheSteps = cachedTasks.length;
    const downloadSteps = downloadTasks.length;
    const totalSteps = cacheSteps + downloadSteps;
    let completeSteps = 0;
    setProgress(0)
    
    if(cacheSteps){ // First pass of cached data
        workers = Array.from({length: Math.min(POOL_COUNT, cacheSteps)}, () => 
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            if (taskIndex >= cacheSteps) return  // this worker is done
            const task = cachedTasks[taskIndex++] // Next isntruction set
            worker.onmessage = () => {
                completed++
                completeSteps++
                setProgress(Math.floor(completeSteps/totalSteps * 100) )
                if (completed === cacheSteps) {
                terminateAll()
                resolve()
            ...
            worker.postMessage({
                ...
                startCoords: [zDim.start,yDim.start,xDim.start],
    ...
    if (downloadSteps){
        workers = Array.from({length: Math.min(POOL_COUNT, downloadSteps)}, () => 
            new Worker(new URL('@/components/workers/fetchWorker.ts', import.meta.url), { type: 'module' })
        )
        let taskIndex = 0
        let completed = 0
    ...
    setProgress(0) // Reset progress for next load
    return {
        data: new Float16Array(sharedBuffer),
        shape: outputShape,
        dtype: outVar.dtype,
        scalingFactor
    }
