...
export const ZARR_STORES = {
	...
	ICON_ESM:
		"https://eerie.cloud.dkrz.de/datasets/icon-esm-er.hist-1950.v20240618.atmos.native.2d_1h_mean/kerchunk",
	OLCI_CHL:
		"https://s3.waw3-2.cloudferro.com/wekeo/egu2025/OLCI_L1_CHL_cube.zarr",
...
function ToFloat16(
	...
	if (maxVal <= 65504 && minVal >= -65504) {
		// If values fit in Float16, use that to save memory
...
function RescaleArray(array: Float16Array, scalingFactor: number) {
...
export function copyChunkToArray(
	...
	for (let cz = 0; cz < zLimit; cz++) {
		for (let cy = 0; cy < yLimit; cy++) {
			...
			const destRowOffset =
				(zStart + cz) * destStride[0] + (yStart + cy) * destStride[1] + xStart;
...
> {
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			const d_store = zarr.tryWithConsolidated(new zarr.FetchStore(storePath));
...
export class ZarrDataset {
	private groupStore: Promise<
		zarr.Group<zarr.FetchStore | zarr.Listable<zarr.FetchStore>>
	>;
	...
	async GetArray(variable: string, slices: Slices) {
		const { is4D, idx4D, initStore, setProgress, setStrides, setStatus } =
			useGlobalStore.getState();
		const { compress, setCurrentChunks, setArraySize } =
			useZarrStore.getState();
		...
		if (
			cache.has(
				is4D ? `${initStore}_${idx4D}_${variable}` : `${initStore}_${variable}`,
			)
		) {
		...
		const hasTimeChunks = is4D
			? outVar.shape[1] / chunkShape[0] > 1
			: outVar.shape[0] / chunkShape[0] > 1;
		...
		if (outVar.is("number") || outVar.is("bigint")) {
			...
			if (!hasTimeChunks) {
				// No chunking along time axis, can download whole array at once
				setStatus("Downloading...");
				for (let attempt = 0; attempt <= maxRetries; attempt++) {
					try {
						chunk = is4D
							? await zarr.get(outVar, [idx4D, null, null, null])
							: await zarr.get(outVar);
				...
				if (
					...
					throw new Error(
						"BigInt arrays are not supported for conversion to Float16Array.",
					);
				} else {
					[typedArray, scalingFactor] = ToFloat16(
				...
				const yShape =
					(ySlice[1] ? ySlice[1] : outVar.shape[1 + (is4D ? 1 : 0)]) -
					ySlice[0];
				const xShape =
					(xSlice[1] ? xSlice[1] : outVar.shape[2 + (is4D ? 1 : 0)]) -
					xSlice[0];
				...
				for (let y = yStartIdx; y < yEndIdx; y++) {
					for (let x = xStartIdx; x < xEndIdx; x++) {
						...
						if (cache.has(cacheName)) {
							...
							const chunkData = cachedChunk.compressed
								? DecompressArray(cachedChunk.data)
								: cachedChunk.data.slice(); // Decompress if needed. Gemini thinks the .slice() helps with garbage collector as it doesn't maintain a reference to the original array
							copyChunkToArray2D(
								chunkData,
								cachedChunk.shape,
								cachedChunk.stride,
							...
							for (let attempt = 0; attempt <= maxRetries; attempt++) {
								try {
									chunk = await zarr.get(outVar, [
										zarr.slice(y * chunkShape[1], (y + 1) * chunkShape[1]),
										zarr.slice(x * chunkShape[2], (x + 1) * chunkShape[2]),
									]);
									break; // If successful, exit the loop
								} catch (error) {
									// If this is the final attempt, handle the error
							...
							if (
								...
								throw new Error(
									"BigInt arrays are not supported for conversion to Float32Array.",
								);
							}
							const originalData = chunk.data as Float32Array;
							const chunkStride = chunk.stride;
							chunk = null; // Clear reference!
							const [chunkF16, newScalingFactor] = ToFloat16(
								originalData,
								scalingFactor,
							);
							if (
								...
								// If the scalingFactor has changed, need to rescale main array
								if (scalingFactor == null || newScalingFactor > scalingFactor) {
									const thisScaling = scalingFactor
										? newScalingFactor - scalingFactor
										: newScalingFactor;
									RescaleArray(typedArray, thisScaling);
									scalingFactor = newScalingFactor;
									for (const id of rescaleIDs) {
										// Set new scalingFactor on the chunks
				...
				setStatus("Downloading...");
				setProgress(0);
				const totalZChunks = Math.ceil(
					outVar.shape[0 + (is4D ? 1 : 0)] / chunkShape[0],
				);
				const totalYChunks = Math.ceil(
					outVar.shape[1 + (is4D ? 1 : 0)] / chunkShape[1],
				);
				const totalXChunks = Math.ceil(
					outVar.shape[2 + (is4D ? 1 : 0)] / chunkShape[2],
				);

				const zStartIdx = Math.floor(zSlice[0] / chunkShape[0]);
				const zEndIdx = zSlice[1]
					? Math.ceil(zSlice[1] / chunkShape[0])
					: totalZChunks; //If Slice[1] is null, use the end of the array
				const yStartIdx = Math.floor(ySlice[0] / chunkShape[1]);
				const yEndIdx = ySlice[1]
					? Math.ceil(ySlice[1] / chunkShape[1])
					: totalYChunks;
				...
				const xEndIdx = xSlice[1]
					...
					: totalXChunks;
				const chunkCount =
					(zEndIdx - zStartIdx) * (yEndIdx - yStartIdx) * (xEndIdx - xStartIdx); // Used for Progress Bar
				const zShape =
					(zSlice[1] ? zSlice[1] : outVar.shape[0 + (is4D ? 1 : 0)]) -
					zSlice[0];
				const yShape =
					(ySlice[1] ? ySlice[1] : outVar.shape[1 + (is4D ? 1 : 0)]) -
					ySlice[0];
				...
				for (let z = zStartIdx; z < zEndIdx; z++) {
					// Iterate through chunks we need
					for (let y = yStartIdx; y < yEndIdx; y++) {
						for (let x = xStartIdx; x < xEndIdx; x++) {
			...
			throw new Error(
				`Unsupported data type: Only numeric arrays are supported. Got: ${outVar.dtype}`,
			);
		}
	}
	...
	async GetAttributes(variable: string) {
		...
		const dims = [];
		for (const dim of meta._ARRAY_DIMENSIONS as string[]) {
			//Put the dimension arrays in the cache to access later
			if (!cache.has(dim)) {
	...
	GetTimeSeries(TimeSeriesInfo: TimeSeriesInfo) {
		...
		const slice = sliceSize.map((value, index) =>
			value === null || shape[index] === null
				? null
				: Math.round(value * shape[index] - 0.5),
		);
