...
const ChunkVisualizer = ({
	...
	const playHeadPos =
		leftPercentage + animProg * (100 - rightPercentage - leftPercentage);
...
const PlayInterFace = ({
	...
	visible: boolean;
	setKeepOpen: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
	const { animate, animProg, zSlice, setAnimate, setAnimProg } = usePlotStore(
		useShallow((state) => ({
	...
	const { dimArrays, dimUnits, zMeta, variable } = useGlobalStore(
		useShallow((state) => ({
	...
	const { reFetch, setZSlice, ReFetch } = useZarrStore(
		useShallow((state) => ({
	...
	const [chunkTimeLength, chunkDivWidth, chunkSize] = useMemo(() => {
		const meta = (
			zMeta as { name: string; chunks: number[]; chunkSize: number }[]
	...
	// LABELS
	const currentLabel = parseLoc(
		timeSlice?.[Math.round(animProg * sliceDist)],
		dimUnits[0],
		true,
	);
				...
				<div className="flex justify-between">
					...
					<Button
						...
						onClick={() => {
							...
							setZSlice([
								zSlice[0],
								Math.min(zSlice[1] + chunkTimeLength, timeLength),
							]);
							setKeepOpen(true);
							AdjustCacheSize();
							ReFetch();
				...
				<div className="grid grid-cols-3 items-center w-full">
					...
					<div className="justify-self-end">
						<Button
							...
							onClick={() =>
								setFPS((x) => Math.min(frameRates.length - 1, x + 1))
							}
							aria-label="Faster"
							title="Faster"
						>
...
const PlayButton = () => {
	const { isFlat, plotOn, dimArrays } = useGlobalStore(
		useShallow((state) => ({
			isFlat: state.isFlat,
			plotOn: state.plotOn,
			dimArrays: state.dimArrays,
