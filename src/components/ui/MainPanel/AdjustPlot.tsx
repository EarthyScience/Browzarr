...
const MinMaxSlider = React.memo(function MinMaxSlider({
	range,
	setRange,
	valueScales,
	min = -1,
...
const DimSlicer = () => {
	...
	const { valueScales, dimArrays, dimNames, dimUnits, is4D } = useGlobalStore(
		useShallow((state) => ({
		...
		<>
			<div className="grid gap-y-[5px] items-center w-50 text-center">
				...
				<Tooltip delayDuration={500}>
					<TooltipTrigger asChild>
						<Button
							...
							onClick={() => setUseFragOpt(!useFragOpt)}
						>
							{useFragOpt ? "Revert to Normal" : "Use Optimized Shader"}
						</Button>
					</TooltipTrigger>
					<TooltipContent side="left">
						Enable this for better performance while navigating. Switch back for
						exports or still images.
					</TooltipContent>
				</Tooltip>
				...
				<div className="grid grid-cols-[auto_60px] items-center text-left">
					...
					<Input
						type="number"
						value={vTransferScale}
						step={0.1}
						min={0}
						onChange={(e) => setVTransferScale(parseFloat(e.target.value))}
					/>
				</div>
				<div className="grid grid-cols-[auto_20%] items-center gap-2 mt-2 text-left">
					<label htmlFor="compress-data">
						<h1>
							...
							<Tooltip>
								...
								<TooltipContent className="max-w-60 break-words whitespace-normal">
									Transparency is scaled from dataset minimum to maximum. Lower
									values are more transparent. When enabled - transparency
									scales based on the cropped values below.
								</TooltipContent>
							</Tooltip>
						</h1>
					</label>
					<Switch
						className="h-5"
						id="compress-data"
						checked={vTransferRange}
						onCheckedChange={(e) => setVTransferRange(e)}
					/>
				</div>
			</div>
...
const PointOptions = () => {
	const { setPointSize, setScaleIntensity, setScalePoints, setTimeScale } =
		usePlotStore(
			useShallow((state) => ({
	...
	const { scalePoints, scaleIntensity, pointSize, timeScale } = usePlotStore(
		useShallow((state) => ({
		...
		<>
			<div className="flex-column items-center w-50 text-center">
				...
				<Button
					...
					size="sm"
					className="w-[100%] cursor-[pointer] mb-2 mt-2"
					onClick={() => setScalePoints(!scalePoints)}
				>
				...
				<div className="relative">
					{timeScale != 1 && (
						<RxReset
							className="text-lg cursor-pointer absolute top-0 left-0 hover:scale-90 transition-transform duration-100 ease-out"
							onClick={(e) => setTimeScale(1)}
						/>
					)}
		...
		<>
			<div className="grid gap-2 mb-2">
				<div
					...
					onClick={() => {
						if (!displaceSurface) {
							setResetCamera(!usePlotStore.getState().resetCamera);
						}
						setDisplaceSurface(!displaceSurface);
						usePlotStore.setState({ rotateFlat: false });
					}}
				>
					<span
						className={`z-10 font-semibold transition-colors ${displaceSurface ? "text-primary" : "text-secondary"}`}
					>
						Flat
					</span>
					<span
				...
				<Hider show={!displaceSurface}>
					<div className="grid gap-2">
						...
						<div className="grid grid-cols-[auto_20%] items-center gap-2 text-left">
							<label htmlFor="offset-switch">
								<b>Offset Negatives</b>
							</label>
							<Switch
								id="offset-switch"
								checked={offsetNegatives}
								onCheckedChange={(e) => setOffsetNegatives(e)}
							/>

							<label htmlFor="rotate-switch">
								<b>Rotate</b>
							</label>
							<Switch
								id="rotate-switch"
								checked={rotateFlat}
								onCheckedChange={(e) =>
									usePlotStore.setState({ rotateFlat: e })
								}
							/>
						</div>
					</div>
				...
				<div
					...
					onClick={() => {
						...
						setDisplacement(
							displacement *
								(displaceSurface
									? maxFaceDisplacement / maxSurfaceDisp
									: maxSurfaceDisp / maxFaceDisplacement),
						);
					}}
				>
					<span
		...
		<div className="grid gap-2 mb-4 justify-items-center ">
			...
			<div className="flex justify-between">
				<div className="flex-col justify-items-center">
					...
					<Input
						value={lonExtent[0]}
						onChange={(e) =>
							setLonExtent([parseFloat(e.target.value), lonExtent[1]])
						}
						type="number"
					/>
				</div>
				<div className="flex-col justify-items-center">
					...
					<Input
						value={lonExtent[1]}
						onChange={(e) =>
							setLonExtent([lonExtent[0], parseFloat(e.target.value)])
						}
						type="number"
					/>
				</div>
			...
			<div className="flex justify-between">
				<div className="flex-col justify-items-center">
					...
					<Input
						value={latExtent[0]}
						onChange={(e) =>
							setLatExtent([parseFloat(e.target.value), latExtent[1]])
						}
						type="number"
					/>
				</div>
				<div className="flex-col justify-items-center">
					...
					<Input
						value={latExtent[1]}
						onChange={(e) =>
							setLatExtent([latExtent[0], parseFloat(e.target.value)])
						}
						type="number"
					/>
				</div>
			...
			<div className="flex justify-between">
				<div className="flex-col justify-items-center">
					...
					<Input
						value={lonResolution}
						onChange={(e) => setLonResolution(parseFloat(e.target.value))}
						type="number"
					/>
				</div>
				<div className="flex-col justify-items-center">
					...
					<Input
						value={latResolution}
						onChange={(e) => setLatResolution(parseFloat(e.target.value))}
						type="number"
					/>
				</div>
			</div>
			...
			<Button
				...
				disabled={
					originalExtent
						.toArray()
						.slice(0, 2)
						.every((val, idx) => val == lonExtent[idx]) &&
					originalExtent
						.toArray()
						.slice(2)
						.every((val, idx) => val == latExtent[idx])
				}
				onClick={(e) => {
			...
			{!isPC && (
				<>
					...
					<div className="grid grid-cols-[auto_20%] items-center gap-2 mt-2 text-left">
						...
						<Switch
							className="h-5"
							id="interpoalte-pixels"
							checked={interpPixels}
							onCheckedChange={(e) => setInterpPixels(e)}
						/>
					</div>
				</>
			)}
			...
			<Popover>
				...
				<PopoverContent
					...
					className={`overflow-y-auto w-[240px] mt-2 mr-1 ${
						popoverSide === "top" ? "max-h-[80vh] mb-1" : "max-h-[70vh]"
					}`}
				>
