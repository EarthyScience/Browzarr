...
const PlotType = () => {
	const { plotType, setPlotType, setUseOrtho } = usePlotStore(
		useShallow((state) => ({
	...
	const { isFlat, variable } = useGlobalStore(
		useShallow((state) => ({
			isFlat: state.isFlat,
		...
		<Popover>
			...
			>
				{plotTypes.map((val, idx) => {
					if (idx < 2 && isFlat) {
						//Hide options not available when flat data
						return null;
					} else {
						...
						if (isLargePointCloud) {
							return (
								<Tooltip key={"pc-warning-tooltip"} open={true}>
									<TooltipTrigger asChild>
										<div key={"pc-warning"}>{button}</div>
									</TooltipTrigger>
