...
function ChunkIDs(
	...
	const zEndIdx = zSlice[1]
		? Math.ceil(zSlice[1] / chunkShape[0])
		: is4D
			? Math.ceil(dataShape[1] / chunkShape[0])
			: Math.ceil(dataShape[0] / chunkShape[0]); //If Slice[1] is null, use the end of the array
	const yStartIdx = Math.floor(ySlice[0] / chunkShape[1]);
	const yEndIdx = ySlice[1]
		? Math.ceil(ySlice[1] / chunkShape[1])
		: is4D
			? Math.ceil(dataShape[2] / chunkShape[1])
			: Math.ceil(dataShape[1] / chunkShape[1]);
	const xStartIdx = Math.floor(xSlice[0] / chunkShape[2]);
	const xEndIdx = xSlice[1]
		? Math.ceil(xSlice[1] / chunkShape[2])
		: is4D
			? Math.ceil(dataShape[3] / chunkShape[2])
			: Math.ceil(dataShape[2] / chunkShape[2]);
	const ids = [];
...
const MetaDataInfo = ({
	...
	const { maxTextureSize, max3DTextureSize } = usePlotStore(
		useShallow((state) => ({
			...
			max3DTextureSize: state.max3DTextureSize,
		})),
	);
	...
	const totalSize = useMemo(
		() => (meta.totalSize ? meta.totalSize : 0),
		[meta],
	);
	const shapeLength = meta.shape.length;
	const zLength = useMemo(
		() => (meta.shape ? meta.shape[shapeLength - 3] : 0),
		[meta],
	);
	const yLength = useMemo(
		() => (meta.shape ? meta.shape[shapeLength - 2] : 0),
		[meta],
	);
	const xLength = useMemo(
		() => (meta.shape ? meta.shape[shapeLength - 1] : 0),
		[meta],
	);
	const is3D = useMemo(
		() => (meta.shape ? meta.shape.length == 3 : false),
		[meta],
	);
	const hasTimeChunks =
		shapeLength > 2
			? meta.shape[shapeLength - 3] / meta.chunks[shapeLength - 3] > 1
			: false;
	const hasYChunks =
		meta.shape[shapeLength - 2] / meta.chunks[shapeLength - 2] > 1;
	const hasXChunks =
		meta.shape[shapeLength - 1] / meta.chunks[shapeLength - 1] > 1;
	...
	const currentSize = useMemo(() => {
		...
		const getSliceDims = (
			...
			return {
				first: sliceFirst,
				last: sliceLast,
				steps: sliceLast - sliceFirst,
			};
		};

		const z = is2D
			? { first: 0, last: 1, steps: 1 }
			: getSliceDims(zSlice, zLength);
				...
				<>
					{is4D && (
						<>
							<div>
								<p>
									This is Four-Dimensional Dataset. You must select an index
									along the first dimension. <br />
									Please select an index from <b>0</b> to{" "}
									<b>{meta.shape[0] - 1}</b>
								</p>
								<Input
									type="number"
					...
					{(is3D || isFlat || idx4D != null) && !(cached && !cachedChunks) && (
						<>
										...
										{hasTimeChunks && (
											<div className="grid gap-1">
												<div className="flex justify-center">
													<b>
														{dimNames[0] == "Default" ? "Axis 0" : dimNames[0]}
													</b>
												</div>
												<SliderThumbs
													min={0}
													...
													value={[
														zSlice[0] ? zSlice[0] : 0,
														zSlice[1] ? zSlice[1] : zLength,
													]}
													step={chunkShape[0]}
													onValueChange={(values: number[]) =>
														setZSlice([values[0], values[1]] as [
															number,
															number | null,
														])
													}
												...
												<div className="grid grid-cols-2">
													<span>
														...
														<input
															...
															value={zSlice[0]}
															onChange={(e) =>
																setZSlice([parseInt(e.target.value), zSlice[1]])
															}
															onBlur={(e) =>
																setZSlice([
																	HandleCustomSteps(
													...
													<span>
														...
														<input
															className="w-[50px]"
															type="number"
															value={zSlice[1] ? zSlice[1] : zLength}
															onChange={(e) =>
																setZSlice([zSlice[0], parseInt(e.target.value)])
															}
															onBlur={(e) =>
																setZSlice([
																	zSlice[0],
										...
										{hasYChunks && (
											<div className="grid gap-1">
												...
												<SliderThumbs
													...
													value={[
														ySlice[0] ? ySlice[0] : 0,
														ySlice[1] ? ySlice[1] : yLength,
													]}
													step={isFlat ? chunkShape[0] : chunkShape[1]}
													onValueChange={(values: number[]) =>
														setYSlice([values[0], values[1]] as [
															number,
															number | null,
														])
													}
												...
												<div className="grid grid-cols-2">
													<span>
														...
														<b>
															{parseLoc(
																dimArrays[isFlat ? 0 : 1]?.[ySlice[0]] ?? null,
																dimUnits[isFlat ? 0 : 1] ?? null,
															)}
														</b>{" "}
														<br /> Index:
														<input
															...
															value={ySlice[0]}
															onChange={(e) =>
																setYSlice([parseInt(e.target.value), ySlice[1]])
															}
															onBlur={(e) =>
																setYSlice([
																	HandleCustomSteps(
													...
													<span>
														...
														<b>
															{parseLoc(
																dimArrays[isFlat ? 0 : 1]?.[
																	ySlice[1] ? ySlice[1] - 1 : yLength - 1
																] ?? null,
																dimUnits[isFlat ? 0 : 1] ?? null,
															)}
														</b>
														...
														<input
															className="w-[50px]"
															type="number"
															value={ySlice[1] ? ySlice[1] : yLength}
															onChange={(e) =>
																setYSlice([ySlice[0], parseInt(e.target.value)])
															}
															onBlur={(e) =>
																setYSlice([
																	ySlice[0],
										...
										{hasXChunks && (
											<div className="grid gap-1">
												...
												<SliderThumbs
													...
													value={[
														xSlice[0] ? xSlice[0] : 0,
														xSlice[1] ? xSlice[1] : xLength,
													]}
													step={isFlat ? chunkShape[1] : chunkShape[2]}
													onValueChange={(values: number[]) =>
														setXSlice([values[0], values[1]] as [
															number,
															number | null,
														])
													}
												...
												<div className="grid grid-cols-2">
													<span>
														...
														<b>
															{parseLoc(
																dimArrays[isFlat ? 1 : 2]?.[xSlice[0]] ?? null,
																dimUnits[isFlat ? 1 : 2] ?? null,
															)}
														</b>{" "}
														<br /> Index:
														<input
															...
															value={xSlice[0]}
															onChange={(e) =>
																setXSlice([parseInt(e.target.value), xSlice[1]])
															}
															onBlur={(e) =>
																setXSlice([
																	HandleCustomSteps(
													...
													<span>
														...
														<b>
															{parseLoc(
																dimArrays[isFlat ? 1 : 2]?.[
																	xSlice[1] ? xSlice[1] - 1 : xLength - 1
																] ?? null,
																dimUnits[isFlat ? 1 : 2] ?? null,
															)}
														</b>
														...
														<input
															className="w-[50px]"
															type="number"
															value={xSlice[1] ? xSlice[1] : xLength}
															onChange={(e) =>
																setXSlice([xSlice[0], parseInt(e.target.value)])
															}
															onBlur={(e) =>
																setXSlice([
																	xSlice[0],
							...
							{currentSize > maxSize && (
								<>
									<div
										className={`flex items-center gap-2 p-2 ${smallCache ? "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800" : "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800"} rounded-md border`}
									>
										<div
											className={`w-2 h-2 ${smallCache ? "bg-red-500" : "bg-emerald-500"} rounded-full`}
										></div>
										<span
											className={`text-xs font-medium ${smallCache ? "text-red-800 dark:text-red-200" : "text-emerald-800 dark:text-emerald-200"}`}
										>
											{smallCache
												? "Selection won't fit in Cache"
												: "Data Will Fit"}
										</span>
									</div>
									<div className="">
										...
										<div className="flex justify-center items-center">
											...
											<Tooltip>
												...
												<TooltipContent>
													Increasing this too far can cause crashes. Mobile
													users beware
												</TooltipContent>
											</Tooltip>
										</div>
							...
							<div className="grid grid-cols-[auto_50%] gap-2 mt-2">
								<div>
									...
									<Tooltip>
										...
										<TooltipContent className="max-w-[min(100%,16rem)] break-words whitespace-normal">
											Compress data to preserve memory at the expense of
								...
								<Switch
									id="compress-data"
									checked={compress}
									onCheckedChange={(e) => setCompress(e)}
								/>
							</div>
						</>
					)}
				...
				{tooBig && (
					<div className="bg-[#FFBEB388] rounded-md p-1">
						<span className="text-xs font-medium text-red-800 dark:text-red-200">
							Not only will this certainly not fit in memory, but it also
							won&apos;t fit in a single shader call. You are wild for this one.
							Textures: <b>{texCount}/14</b>
						</span>
					</div>
				)}
				{!tooBig && (
