...
const webGPUError = (
	<div className="m-0 p-5 font-sans flex-column justify-center items-center">
		...
		<p className="text-base leading-relaxed mb-1 opacity-95">
			WebGPU is not supported or enabled in your current browser. This feature
			is required for GPU-accelerated computing.
		</p>
...
const AnalysisOptions = () => {
	...
	useEffect(() => {
		// Changing stores makes it so you can't use two variable operations.
	...
	useEffect(() => {
		// When data is downloaded (indicated by changes in refetch) The newly plotted and any future variables are compatible until initStore changes.
		setIncompatible(false);
						...
						<>
							...
							<table style={{ textAlign: "right" }}>
								<tbody>
													...
													<div className="rounded-[6px] self-center mx-2 relative border border-gray-150 py-[5px] px-1">
														<div className="flex justify-around">
															...
															<Tooltip>
																...
																<TooltipContent>
																	Operations will be applied to the newly
																	generated data.
																</TooltipContent>
															</Tooltip>
														</div>
											...
											<tr>
												...
												<td>
													<Select onValueChange={setVariable2}>
														<SelectTrigger
															style={{ width: "175px", marginLeft: "18px" }}
														>
															<SelectValue placeholder={"Select..."} />
														</SelectTrigger>
														<SelectContent>
															{variables.map((iVar, idx) => {
																//Dont allow correlation of two variables
																if (iVar == variable) {
																	return null;
																}
																return (
									...
									{[
										...singleVarReductionOps.map((op) => op.value),
										...multiVar2DOps.map((op) => op.value),
										"CUMSUM3D",
									].includes(operation) &&
										!isFlat && (
											<tr>
												...
												>
													<Select onValueChange={(e) => setNewDim(parseInt(e))}>
														<SelectTrigger
															style={{
																width: ["CUMSUM3D", "LinearSlope"].includes(
																	operation,
																)
																	? "50%"
																	: "175px",
													...
													{["CUMSUM3D"].includes(operation) && (
														<Tooltip delayDuration={300}>
															...
															<TooltipContent side="bottom">
																Reverse the direction of the operation along the
																axis
															</TooltipContent>
														</Tooltip>
													)}
									...
									{operation == "Convolution" && (
										<>
															...
															<tr>
																...
																>
																	<Input
																		...
																		onBlur={(e) =>
																			setKernelSize(
																				HandleKernelNums(e.target.value),
																			)
																		}
																	/>
																</td>
																{!isFlat && (
																	...
																	>
																		<Input
																			...
																			min="1"
																			step="2"
																			value={String(kernelDepth)}
																			onChange={(e) =>
																				setKernelDepth(parseInt(e.target.value))
																			}
																			onBlur={(e) =>
																				setKernelDepth(
																					HandleKernelNums(e.target.value),
											...
											<tr>
												...
												<th>
													<KernelVisualizer
														size={Math.min(kernelSize, 15)}
														depth={Math.min(kernelDepth, 15)}
													/>
												</th>
											</tr>
										</>
