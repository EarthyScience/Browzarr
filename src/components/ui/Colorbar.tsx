...
const Colorbar = ({
	...
	const { colormap, variable } = useGlobalStore(
		useShallow((state) => ({
			colormap: state.colormap,
			variable: state.variable,
		})),
	);
	const { cScale, cOffset, setCScale, setCOffset } = usePlotStore(
		useShallow((state) => ({
			cScale: state.cScale,
	...
	const { variable2, analysisMode, operation, kernelOperation, execute } =
		useAnalysisStore(
			useShallow((state) => ({
	...
	const prevPos = useRef<{ x: number | null; y: number | null }>({
		x: null,
		y: null,
	});
	...
	const prevVals = useRef<{ min: number | null; max: number | null }>({
		min: null,
		max: null,
	});
	...
	useEffect(() => {
		// Update internal vals when global vals change
		setNewMin(origMin);
		setNewMax(origMax);
	}, [origMax, origMin]);
	...
	const analysisString = useMemo(() => {
		if (analysisMode) {
			...
			const thisOperation =
				operation === "Convolution" ? kernelOperation : operation;
			const theseUnits =
				operationMap[thisOperation as keyof typeof operationMap];
			const string = twoVar
				? `+ ${variable2} (${theseUnits})`
				: `[${units}] (${theseUnits})`;
			return string;
		} else {
			return `[${units}]`;
		...
		<>
			<div className="colorbar">
				...
				{(cScale != 1 || cOffset != 0) && (
					<RxReset
						size={25}
						style={{ position: "absolute", top: "-25px", cursor: "pointer" }}
						onClick={() => {
							setNewMin(origMin);
							setNewMax(origMax);
						}}
				...
				>
					<FaMinus
						...
						onClick={() => setTickCount(Math.max(tickCount - 1, 2))}
					/>
					<FaPlus
						className="cursor-pointer"
						onClick={() => setTickCount(Math.min(tickCount + 1, 10))}
					/>
				</div>
			</div>
		</>
