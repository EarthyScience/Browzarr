...
export default function VariablesTable() {
	...
	const filtered = useMemo(() => {
		...
		return variables.filter((variable) => variable.toLowerCase().includes(q));
	}, [query, variables]);
									...
									filtered.map((variable, idx) => (
										<TableRow key={idx} className="hover:bg-gray">
											<TableCell className="text-center">
												<Dialog
													open={
														showMetaDialog && selectedMeta?.name === variable
													}
													onOpenChange={(open) => {
														if (!open) {
												...
												>
													...
													<DialogContent
														aria-describedby="Data information for selected variable"
														aria-description="Data information for selected variable"
														className="max-w-[85%] md:max-w-2xl max-h-[80vh] overflow-y-auto"
													>
