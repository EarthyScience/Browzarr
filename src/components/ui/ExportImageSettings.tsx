...
const ExportImageSettings = () => {
	const {
		...
		useShallow((state) => ({
			...
			cbarLabel: state.cbarLabel,
			cbarUnits: state.cbarUnits,
	...
	const { plotType, zSlice } = usePlotStore(
		useShallow((state) => ({
			plotType: state.plotType,
			zSlice: state.zSlice,
		})),
	);
	const { variable, metadata, dimArrays } = useGlobalStore(
		useShallow((state) => ({
	...
	const capitalize: CapitalizeFn = (str) =>
	...
	return (
		<Popover>
			...
			<PopoverContent side="right" className="w-[200px] select-none">
				<div className="grid items-center gap-2">
					...
					<Hider show={showTitles} className="col-span-2">
						<div className="grid">
							...
							<Input
								id="cbar-title"
								type="string"
								placeholder={metadata?.units ?? "undefined"}
								value={cbarUnits}
								onChange={(e) => setCbarUnits(e.target.value)}
							/>
						</div>
					</Hider>
					{/* Settings */}
					...
					<Hider show={showSettings} className="col-span-2">
						<div className="grid grid-cols-[auto_60px] items-center gap-2">
							...
							<Switch
								...
								onCheckedChange={(e) => setIncludeBackground(e)}
							/>
							{plotType != "sphere" && (
								<>
									...
									<Switch
										id="includeBG"
										checked={includeAxis}
										onCheckedChange={(e) => setIncludeAxis(e)}
									/>
								</>
							)}
							...
							<Switch
								...
								onCheckedChange={(e) => setIncludeColorbar(e)}
							/>

							<Hider show={includeColorbar} className="col-span-2">
								<div className="col-span-2 flex justify-between">
									<label htmlFor="colorbar-loc ">
										Colorbar <br /> Location
									</label>
									<div id="colorbar-loc">
										<Select
											value={cbarLoc}
											onValueChange={(e) => setCbarLoc(e)}
										>
											<SelectTrigger>
								...
								<div className="grid grid-cols-[auto_60px] items-center gap-2">
									...
									<Input
										...
										step={1}
										value={cbarNum}
										onChange={(e) => setCbarNum(parseInt(e.target.value))}
									/>
								</div>
							...
							<Switch
								...
								onCheckedChange={(e) => setUseCustomRes(e)}
							/>
								...
								<div className="grid grid-cols-[50%_50%] col-span-2 ">
									<div className="flex flex-col items-center">
										...
										<Input
											...
											value={customRes[0]}
											onChange={(e) =>
												setCustomRes([parseInt(e.target.value), customRes[1]])
											}
										/>
									</div>
									<div className="flex flex-col items-center">
										...
										<Input
											...
											value={customRes[1]}
											onChange={(e) =>
												setCustomRes([customRes[0], parseInt(e.target.value)])
											}
										/>
									</div>
								</div>
							...
							<Hider show={!useCustomRes} className="col-span-2">
								<div className="grid grid-cols-[auto_60px] items-center gap-2">
									...
									<Switch
										id="includeCbar"
										checked={doubleSize}
										onCheckedChange={(e) => setDoubleSize(e)}
									/>
								</div>
							</Hider>
						</div>
					...
					<Hider show={showAnimation} className="grid col-span-2 gap-2">
						<div className="grid grid-cols-[auto_60px] items-center gap-2">
							...
							<Switch
								id="useAnimate"
								checked={animate}
								onCheckedChange={(e) => setAnimate(e)}
							/>
							{/* Animation Settings */}
							<Hider show={animate} className="col-span-2 ">
								...
								<div className="grid grid-cols-[auto_60px] items-center gap-2">
									...
									<Switch
										id="useOrbit"
										checked={pingpong}
										onCheckedChange={(e) => setPingpong(e)}
									/>
									...
									<Switch
										id="useTime"
										checked={useTime}
										onCheckedChange={(e) => setUseTime(e)}
									/>
									<Hider show={useTime} className="col-span-2">
										<div className="grid grid-cols-[auto_80px] items-center gap-2 mb-2">
											...
											<Input
												...
												step={1}
												value={timeRate}
												onChange={(e) => setTimeRate(parseInt(e.target.value))}
											/>
										</div>
										<div className="grid grid-cols-[auto_60px] items-center gap-2">
											...
											<Switch
												id="loopTime"
												checked={loopTime}
												onCheckedChange={(e) => setLoopTime(e)}
											/>
										</div>
										<div className="border-b my-2" />
									</Hider>
									...
									<Switch
										id="changeViz"
										checked={animViz}
										onCheckedChange={(e) => setAnimViz(e)}
									/>
								...
								<div className="grid grid-cols-[auto_60px] items-center gap-2 my-2">
									...
									<Switch
										id="usePreview"
										checked={preview}
										onCheckedChange={(e) => setPreview(e)}
									/>
								</div>
							</Hider>
						</div>
					...
					<Button
						...
						onClick={(e) => {
							ExportImg();
							setHideAxisControls(true);
							setHideAxis(!includeAxis);
						}}
					>
						Export
					</Button>
