...
function StoreData(
	...
	const [minVal, maxVal] = valueScales
		? [valueScales.minVal, valueScales.maxVal]
		: ArrayMinMax(data);
...
export function CreateTexture(
	shape: number[],
	data?: Uint8Array,
): THREE.DataTexture[] | THREE.Data3DTexture[] | undefined {
	...
	if (shape.length == 2) {
		...
		const chunkData = chunkArray2D(
			textureData as Uint8Array,
			{ y: height, x: width },
			chunkSize,
		);
		const chunks = [];
...
export function ArrayToTexture(
	array: Array,
	valueScales?: { maxVal: number; minVal: number },
): [
			...
			for (let cx = 0; cx < resolution[2]; cx++) {
				...
				for (let z = startZ; z < endZ; z++) {
					for (let y = startY; y < endY; y++) {
						...
						chunk.set(
							arr.subarray(rowOffset, rowOffset + rowLength),
							chunkOffset,
						);
				...
				chunks.push({
					data: chunk,
					dims: { x: rowLength, y: chunkHeight, z: chunkDepth },
				});
			}
		}
	}
