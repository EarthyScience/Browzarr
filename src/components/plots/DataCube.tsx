...
export const DataCube = ({ volTexture }: DataCubeProps) => {
	const { shape, colormap, flipY, textureArrayDepths } = useGlobalStore(
		useShallow((state) => ({
			shape: state.shape,
				...
				uniforms: {
					...
					vertBounds: {
						value: new THREE.Vector2(
							yRange[0] * aspectRatio,
							yRange[1] * aspectRatio,
						),
					},
					steps: { value: quality },
	...
	useFrame(({ camera }) => {
		// This calculates InverseModel matrix for the orthographic raymarcher
		if (!useOrtho || !meshRef.current || !shaderMaterial) return;
		meshRef.current.modelViewMatrix.multiplyMatrices(
