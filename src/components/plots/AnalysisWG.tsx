...
const AnalysisWG = ({
	...
	setTexture: React.Dispatch<
		React.SetStateAction<THREE.Data3DTexture[] | THREE.DataTexture[] | null>
	>;
	ZarrDS: ZarrDataset;
}) => {
	...
	const { zSlice, ySlice, xSlice } = usePlotStore(
		useShallow((state) => ({
			zSlice: state.zSlice,
			ySlice: state.ySlice,
			xSlice: state.xSlice,
		...
		const executeAnalysis = async () => {
			...
			const currentOperation =
				operation == "Convolution"
					? (kernelOperation as Operation)
					: (operation as Operation); // If it's convolution use the kernelOperation
			...
			if (useTwo) {
				...
				const var2Array = await ZarrDS.GetArray(variable2, {
					zSlice,
					ySlice,
					xSlice,
				});
			...
			switch (currentOperation) {
				...
				case "StDev2D":
					const shape2D =
						dataShape.length > 2
							? dataShape.filter((_, idx) => idx !== analysisDim)
							: dataShape;
					const strides2D = strides.length > 2 ? [shape2D[1], 1] : strides;
					newArray = await Convolve2D(
						inputArray,
				...
				case "Correlation2D":
				case "TwoVarLinearSlope2D":
				case "Covariance2D":
					newArray = await Multivariate2D(
						inputArray,
						var2Data!,
						shapeInfo,
						axis,
				...
				case "Correlation3D":
				case "TwoVarLinearSlope3D":
				case "Covariance3D":
					newArray = await Multivariate3D(
						inputArray,
						var2Data!,
						shapeInfo,
						kernelParams,
			...
			const needsRescale = [
				...
				"Covariance",
				"CUMSUM3D",
			].some((op) => currentOperation.includes(op));
			...
			const thisShape =
				dataShape.length > 2
					? dataShape.filter((_, idx) => idx !== axis)
					: dataShape;
			...
			const newTexture = CreateTexture(
				is3DResult ? dataShape : thisShape,
				textureData,
			);
			// --- Final state updates ---
