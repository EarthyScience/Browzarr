	...
	const textArray = useMemo(() => {
		...
		if (xDimArray) {
			...
			if (isBig) {
				...
				const [unit, offset] = firstID
					? parseTimeUnit(coords[firstID].plot.units)
					: [1, 0];
				const timeStrings = [];
			...
			{showGrid && (
				<>
					...
					{Array.from({ length: xTickCount }, (_, i) => {
						...
						const x =
							Math.round(bounds.left / stickyLines) * stickyLines +
							(Math.round(bounds.right / stickyLines) * stickyLines -
								Math.round(bounds.left / stickyLines) * stickyLines) *
								(i / (xTickCount - 1));
						const normX =
							x / xScale / (initialBounds.right - initialBounds.left) + 0.5;
						const y = vertY;
						return (
								...
								<group key={`top-tick-${i}`} position={[x, bounds.top, 0]}>
									<line>
										<bufferGeometry>
											<float32BufferAttribute
												attach="attributes-position"
												args={[
													new Float32Array([0, 0, 0, 0, -sizes.tickSize, 0]),
													3,
												]}
											/>
										</bufferGeometry>
									...
									{i !== 0 && i !== xTickCount - 1 && (
										<Text
											...
											anchorY="top"
										>
											{textArray?.[Math.round(normX * xDimSize - 0.5)] ?? ""}
										</Text>
									)}
								</group>
							</Fragment>
					...
					{Array.from({ length: yTickCount }, (_, i) => {
						...
						const y =
							bounds.bottom +
							(bounds.top - bounds.bottom) * (i / (yTickCount - 1));
						const normY =
							y / yScale / (bounds.top - bounds.bottom) / zoom + 0.5;
						const x = horX;
						return (
							<Fragment key={`vert-group-${i}`}>
									...
									<line>
										<bufferGeometry>
											<float32BufferAttribute
												attach="attributes-position"
												args={[
													new Float32Array([0, 0, 0, -sizes.tickSize, 0, 0]),
													3,
												]}
											/>
										</bufferGeometry>
