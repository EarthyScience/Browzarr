...
function PlotPoints({
	...
	scalers: { xScale: number; yScale: number };
}) {
	...
	const { pointColor, pointSize, useCustomPointColor } = usePlotStore(
		useShallow((state) => ({
	...
	const { timeSeries } = useGlobalStore(
		useShallow((state) => ({
		...
		() =>
			new THREE.MeshBasicMaterial({
				color: new THREE.Color()
					.setRGB(r / 300, g / 300, b / 300)
					.convertSRGBToLinear(),
			}),
		[pointColor, useCustomPointColor, timeSeries],
	); // It was converting to sRGB colorspace while the line shader uses linear
	...
	function scalePoint(e: any) {
		if (ref.current) {
			...
			if (thisID != lastID.current && lastID.current) {
				//Revert last big point to small
	...
	return (
		<>
			<mesh
				position={[0, 0, 5]}
				onPointerEnter={scalePoint}
				onPointerLeave={restorePoint}
			>
