...
function PointInfo({ pointID, pointLoc, showPointInfo, plotUnits }: pointInfo) {
	...
	let pointX = 0;
	if (
		Object.entries(pointID).length > 0 &&
...
function PointCoords({ open, height }: { open: boolean; height: number }) {
	const { coords, timeSeries, setDimCoords, setTimeSeries } = useGlobalStore(
		useShallow((state) => ({
	...
	const initialMouse = useRef<number[]>([
		0,
		Math.round(window.innerHeight * 0.255),
	]);
	const initialDiv = useRef<number[]>([
		0,
		Math.round(window.innerHeight * 0.255),
	]);
		...
		<>
			<div
				...
				style={{
					...
					transform: open
						? ""
						: `translateX(${(moveLeft ? -1 : 1) * window.innerWidth}px) translateY(${-height}px)`,
...
export function PlotArea() {
	...
	const [height, setHeight] = useState<number>(
		Math.round(window.innerHeight - window.innerHeight * 0.25),
	);
	const { metadata, timeSeries } = useGlobalStore(
		useShallow((state) => ({
