...
const FlatMap = ({
	...
	textures: THREE.DataTexture | THREE.Data3DTexture[];
	infoSetters: InfoSettersProps;
	ZarrDS: ZarrDataset;
}) => {
	...
	const { axis, analysisMode, analysisArray } = useAnalysisStore(
		useShallow((state) => ({
			axis: state.axis,
	...
	const geometry = useMemo(
		() => new THREE.PlaneGeometry(2, 2 * shapeRatio),
		[shapeRatio],
	);
	...
	const sampleArray = useMemo(
		() => (analysisMode ? analysisArray : GetCurrentArray()),
		[analysisMode, analysisArray, textures],
	);
	const analysisDims = useMemo(
		() =>
			dimArrays.length > 2
				? dimSlices.filter((_e, idx) => idx != axis)
				: dimSlices,
		[dimSlices, axis],
	);
	useEffect(() => {
	...
	const handleMove = (e: ThreeEvent<PointerEvent>) => {
		if (infoRef.current && e.uv) {
			...
			const xSize = isFlat
				? analysisMode
					? analysisDims[1].length
					: dimSlices[1].length
				: dimSlices[2].length;
			const ySize = isFlat
				? analysisMode
					? analysisDims[0].length
					: dimSlices[0].length
				: dimSlices[1].length;
			const xIdx = Math.round(x * xSize - 0.5);
			const yIdx = Math.round(y * ySize - 0.5);
			let dataIdx = xSize * yIdx + xIdx;
			dataIdx += isFlat
				? 0
				: Math.floor((dimSlices[0].length - 1) * animProg) * xSize * ySize;
			const dataVal = sampleArray ? sampleArray[dataIdx] : 0;
			val.current = dataVal;
			coords.current = isFlat
				? analysisMode
					? [analysisDims[0][yIdx], analysisDims[1][xIdx]]
					: [dimSlices[0][yIdx], dimSlices[1][xIdx]]
				: [dimSlices[1][yIdx], dimSlices[2][xIdx]];
		}
	};
	...
	const [height, width] = [
		dataShape[dataShape.length - 2],
		dataShape[dataShape.length - 1],
	];
	...
	return (
		<>
			<mesh
				...
				geometry={geometry}
				scale={[
					analysisMode && axis == 2 && flipY ? -1 : 1,
					flipY ? -1 : analysisMode && axis == 2 ? -1 : 1,
					1,
				]}
