...
function Borders({ features }: { features: any }) {
	...
	const { flipY, shape } = useGlobalStore(
		useShallow((state) => ({
			flipY: state.flipY,
			shape: state.shape,
		})),
		...
		() =>
			new THREE.ShaderMaterial({
				...
				uniforms: {
					...
					yBounds: {
						value: new THREE.Vector2(yRange[0] / shape.x, yRange[1] / shape.x),
					},
	...
	useEffect(() => {
		if (lineShaderMat) {
			...
			uniforms.yBounds.value = new THREE.Vector2(
				yRange[0] / shape.x,
				yRange[1] / shape.x,
			);
...
const CountryBorders = () => {
	...
	const { dataShape, is4D } = useGlobalStore(
		useShallow((state) => ({
			dataShape: state.dataShape,
			is4D: state.is4D,
		})),
	);
	const { zRange, plotType, showBorders, timeScale, rotateFlat, pointSize } =
		usePlotStore(
			useShallow((state) => ({
				zRange: state.zRange,
				plotType: state.plotType,
	...
	return (
		<group
			...
			scale={[
				globalScale,
				globalScale * (spherize ? 1 : aspectRatio),
				globalScale,
			]}
		>
			<group
				...
				position={
					spherize || isFlatMap
						? [0, 0, isFlatMap ? 0.001 : 0]
						: [
								0,
								0,
								swapSides
									? zRange[0] *
										(isPC ? depthScale + pointSize / 10000 : timeRatio / 2)
									: zRange[1] *
										(isPC ? depthScale + pointSize / 10000 : timeRatio / 2),
							]
				} // I don't know what value to use here. THis seems okay but not perfect
			>
