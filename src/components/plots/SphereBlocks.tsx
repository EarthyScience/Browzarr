...
const SphereBlocks = ({
	...
	const { colormap, isFlat, valueScales, dataShape, textureArrayDepths } =
		useGlobalStore(
			useShallow((state) => ({
	...
	const geometry = useMemo(() => {
		...
		geo.setAttribute("instanceUV", new THREE.InstancedBufferAttribute(uvs, 2));
		return geo;
	}, [dataShape]);
	...
	const [lonBounds, latBounds] = useMemo(() => {
		...
		const newLonBounds = [
			Math.max(lonExtent[0] - newLonStep, -180),
			Math.min(lonExtent[1] + newLonStep, 180),
		];
		const newLatBounds = [
			Math.max(latExtent[0] - newLatStep, -90),
			Math.min(latExtent[1] + newLatStep, 90),
		];
	...
	useEffect(() => {
		if (shaderMaterial) {
			...
			uniforms.displaceZero.value = offsetNegatives
				? 0
				: -valueScales.minVal / (valueScales.maxVal - valueScales.minVal);
		}
		invalidate();
	...
	const nanMaterial = useMemo(
		() => new THREE.MeshBasicMaterial({ color: nanColor }),
		[],
	);
	nanMaterial.transparent = true;

	const nanSphereGeometry = useMemo(
