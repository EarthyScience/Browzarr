...
const MappingCube = ({
	...
	const { timeScale, zSlice, ySlice, xSlice, getColorIdx, incrementColorIdx } =
		usePlotStore(
			useShallow((state) => ({
	...
	function HandleTimeSeries(event: THREE.Intersection) {
		...
		if (ZarrDS) {
			...
			let dimCoords = coordUV.map((val, idx) =>
				val
					? dimSlices[idx][Math.round(val * dimSlices[idx].length - 0.5)]
					: null,
			);
			...
			const dims = [depth, height, width].filter(
				(_, idx) => coordUV[idx] != null,
			);
			const dimWidth = [depth, height, width].filter(
				(_, idx) => coordUV[idx] == null,
			);
	...
	return (
		<mesh
			scale={[
				2 * globalScale,
				2 * shapeRatio * globalScale,
				2 * depthRatio * globalScale,
			]}
...
export const PointCloud = ({
	...
	const { timeSeries, flipY, dataShape, textureData } = useGlobalStore(
		useShallow((state) => ({
	...
	useEffect(() => {
		//This goes through the list of highlighted columns and removes those that aren't included in the timeseries object.
	...
	const geometry = useMemo(() => {
		...
		geom.setAttribute(
			"value",
			new THREE.Uint8BufferAttribute(data as Uint8Array, 1),
		);
		...
		() =>
			new THREE.ShaderMaterial({
				...
				uniforms: {
					...
					cScale: { value: cScale },
					valueRange: {
						value: new THREE.Vector2(valueRange[0], valueRange[1]),
					},
	...
	useEffect(() => {
		if (shaderMaterial) {
			...
			uniforms.flatBounds.value.set(xRange[0], xRange[1], zRange[0], zRange[1]);
