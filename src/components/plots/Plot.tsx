...
const TransectNotice = () => {
	const { selectTS } = usePlotStore(
		useShallow((state) => ({ selectTS: state.selectTS })),
	);

	return (
		<>
			{selectTS && <div className="transect-notice">Transect Select Mode</div>}
		</>
	);
};

const Orbiter = ({ isFlat }: { isFlat: boolean }) => {
	const { resetCamera, useOrtho, displaceSurface } = usePlotStore(
		useShallow((state) => ({
	...
	const { setCameraRef } = useImageExportStore(
		useShallow((state) => ({ setCameraRef: state.setCameraRef })),
	);
		...
		if (hasMounted.current) {
			...
			if (useOrtho) {
				...
				newCamera.position.copy(
					camera.position.normalize().multiply(new THREE.Vector3(4, 4, 4)),
				); // 4 seems like good distance
...
const Plot = ({ ZarrDS }: { ZarrDS: ZarrDataset }) => {
	...
	const { colormap, variable, isFlat, DPR, valueScales, is4D, setIsFlat } =
		useGlobalStore(
			useShallow((state) => ({
	...
	const { plotType, displaceSurface, interpPixels, setPlotType } = usePlotStore(
		useShallow((state) => ({
	...
	const [textures, setTextures] = useState<
		THREE.DataTexture[] | THREE.Data3DTexture[] | null
	>(null);
			...
			try {
				...
				ZarrDS.GetArray(variable, { xSlice, ySlice, zSlice }).then((result) => {
					...
					if (result.scalingFactor) {
						...
						setValueScales({
							maxVal: maxVal * Math.pow(10, result.scalingFactor),
							minVal: minVal * Math.pow(10, result.scalingFactor),
						});
					} else {
					...
					if (shapeLength == 2) {
						...
						if (!["flat", "sphere"].includes(plotType)) {
							// If the plottype isn't already sphere or flat, change it to sphere
							setPlotType("sphere");
						}
					} else {
						setIsFlat(false);
					...
					const aspectRatio =
						result.shape[shapeLength - 2] / result.shape[shapeLength - 1];
					const timeRatio =
						result.shape[shapeLength - 3] / result.shape[shapeLength - 1];
					setShape(
						new THREE.Vector3(2, aspectRatio * 2, Math.max(timeRatio, 2)),
					);
	...
	useEffect(() => {
		// Rotates flat back when changing away
		...
		>
			...
			{(isFlat || plotType == "flat") && (
				<AnalysisInfo
					loc={loc}
					show={showInfo}
					info={[...coords.current, val.current]}
				/>
			)}
			<Canvas
				id="main-canvas"
				camera={{ position: isFlat ? [0, 0, 5] : [-4.5, 3, 4.5], fov: 50 }}
				frameloop="demand"
			...
			>
				...
				{plotType == "point-cloud" && show && (
					<>
						<PointCloud
							textures={{
								texture: textures as THREE.Data3DTexture[],
								colormap,
							}}
							ZarrDS={ZarrDS}
						/>
				...
				{plotType == "flat" && show && (
					<>
						{displaceSurface && (
							<FlatMap
								textures={textures as THREE.DataTexture | THREE.Data3DTexture[]}
