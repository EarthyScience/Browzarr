...
function XYZtoUV(xyz: THREE.Vector3, width: number, height: number) {
	...
	u = Math.round(u * width - 0.5) / width;
	v = Math.round(v * height - 0.5) / height;
...
function XYZtoRemap(
	...
	const lat = Math.asin(xyz.y);
	const u =
		(lon - deg2rad(lonBounds[0])) /
		(deg2rad(lonBounds[1]) - deg2rad(lonBounds[0]));
	const v =
		(lat - deg2rad(latBounds[0])) /
		(deg2rad(latBounds[1]) - deg2rad(latBounds[0]));
...
export const Sphere = ({
	...
	textures: THREE.Data3DTexture[] | THREE.DataTexture[] | null;
	ZarrDS: ZarrDataset;
}) => {
	const { setPlotDim, updateDimCoords, updateTimeSeries } = useGlobalStore(
		useShallow((state) => ({
	...
	const [bounds, setBounds] = useState<THREE.Vector4[]>(
		new Array(10).fill(new THREE.Vector4(-1, -1, -1, -1)),
	);
	const [height, width] = useMemo(
		() => (isFlat ? dataShape : [dataShape[1], dataShape[2]]),
		[dataShape],
	);

	useEffect(() => {
		//This goes through the list of highlighted squares and removes those that aren't included in the timeseries object.
		...
		const paddedArray = [
			...pointValues,
			...Array(Math.max(0, 10 - pointValues.length)).fill(
	...
	function addBounds(uv: THREE.Vector2, tsID: string) {
		//This adds the bounds in UV space of a selected square on the sphere.
		const widthID = Math.floor(uv.x * width) + 0.5;
		const heightID = Math.ceil(uv.y * height) - 0.5;
		const delX = 1 / width;
		const delY = 1 / height;
		const xBounds = [widthID / width - delX / 2, widthID / width + delX / 2];
		const yBounds = [
			heightID / height - delY / 2,
			heightID / height + delY / 2,
		];
		const bounds = new THREE.Vector4(...xBounds, ...yBounds);
		const newBoundObj = { [tsID]: bounds };
		setBoundsObj((prev) => {
			return { ...newBoundObj, ...prev };
		});
	}

	const [lonBounds, latBounds] = useMemo(() => {
		//The bounds for the shader. It takes the middle point of the furthest coordinate and adds the distance to edge of pixel
	...
	const geometry = useMemo(
		() => new THREE.IcosahedronGeometry(1, sphereResolution),
		[sphereResolution],
	);
			...
			uniforms: {
				...
				textureDepths: {
					value: new THREE.Vector3(
						textureArrayDepths[2],
						textureArrayDepths[1],
						textureArrayDepths[0],
					),
				},
				...
				displaceZero: {
					value:
						-valueScales.minVal / (valueScales.maxVal - valueScales.minVal),
				},
	...
	useEffect(() => {
		if (shaderMaterial) {
			...
			uniforms.latBounds.value = new THREE.Vector2(
				deg2rad(latBounds[0]),
				deg2rad(latBounds[1]),
			);
			uniforms.lonBounds.value = new THREE.Vector2(
				deg2rad(lonBounds[0]),
				deg2rad(lonBounds[1]),
			);
			uniforms.nanColor.value = new THREE.Color(nanColor);
			uniforms.nanAlpha.value = 1 - nanTransparency;
			uniforms.displaceZero.value =
				-valueScales.minVal / (valueScales.maxVal - valueScales.minVal);
	...
	function HandleTimeSeries(event: THREE.Intersection) {
		...
		if (ZarrDS) {
			...
			const tempTS = GetTimeSeries(
				{
					data: analysisMode ? analysisArray : GetCurrentArray(),
					shape: dataShape,
					stride: strides,
				},
				{ uv: tsUV, normal },
			...
			let dimCoords = coordUV.map((val, idx) =>
				val ? dimSlices[idx][Math.round(val * dimSlices[idx].length)] : null,
			);
	...
	return (
		<>
			<mesh
				...
				onClick={(e) => selectTS && HandleTimeSeries(e)}
			/>
