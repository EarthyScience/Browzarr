...
export async function DataReduction(
	inputArray: ArrayBufferView,
	dimInfo: { shape: number[]; strides: number[] },
	reduceDim: number,
	operation: string,
) {
	const { device, hasF16 } = await InitializeDevice();
	if (!device) {
		// Redundant check but needed to satisfy typescript that device is not undefined
		Error("need a browser that supports WebGPU");
		return;
	}
	...
	const shaderKey = ShaderMap[
		operation as keyof typeof ShaderMap
	] as keyof typeof shaders;
	...
	device.queue.writeBuffer(
		...
		(hasF16
			? inputArray
			: new Float32Array(
					inputArray as Float16Array,
				)) as GPUAllowSharedBufferSource,
	);
	device.queue.writeBuffer(
		uniformBuffer,
		0,
		myUniformValues.arrayBuffer as GPUAllowSharedBufferSource,
	);
	...
	const results = hasF16
		? new Float16Array(resultArrayBuffer.slice())
		: new Float16Array(new Float32Array(resultArrayBuffer.slice()));
...
export async function Multivariate2D(
	firstArray: ArrayBufferView,
	secondArray: ArrayBufferView,
	dimInfo: { shape: number[]; strides: number[] },
	reduceDim: number,
	operation: string,
) {
...
export async function Multivariate3D(
	firstArray: ArrayBufferView,
	secondArray: ArrayBufferView,
	dimInfo: { shape: number[]; strides: number[] },
	kernel: { kernelSize: number; kernelDepth: number },
...
export async function CUMSUM3D(
	inputArray: ArrayBufferView,
	dimInfo: { shape: number[]; strides: number[] },
	reduceDim: number,
	reverse: number,
	...
	encoder.copyBufferToBuffer(outputBuffer, 0, readBuffer, 0, outputSize * 4);
