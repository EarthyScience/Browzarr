...
export function parseTimeUnit(units: string | undefined): [number, number] {
	...
	if (!match) {
		throw new Error(
			`Invalid time unit format: expected "<unit> since <date>", got "${units}"`,
		);
	}
	...
	const singularUnit = normalizedUnit.endsWith("s")
		...
		: normalizedUnit;
	const effectiveUnit =
		unitToMilliseconds[normalizedUnit] !== undefined
			? normalizedUnit
			: singularUnit;
	...
	if (!(effectiveUnit in unitToMilliseconds)) {
		throw new Error(
			`Unsupported time unit: "${unit}". Supported units: ${Object.keys(unitToMilliseconds).join(", ")}`,
		);
	}
...
export function getUnitAxis(vec: THREE.Vector3) {
	//Takes the normal of a cube interaction to figure out which axis to move along within the data for the timeseries
...
export async function getVariablesOptions(
	variablesPromise: Promise<string[]> | undefined,
) {
...
export function GetCurrentArray(overrideStore?: string) {
	const { variable, is4D, idx4D, initStore, strides, dataShape, setStatus } =
	...
	if (
		cache.has(is4D ? `${store}_${idx4D}_${variable}` : `${store}_${variable}`)
	) {
		const chunk = cache.get(
			is4D ? `${store}_${idx4D}_${variable}` : `${store}_${variable}`,
		);
		...
		for (let z = zStartIdx; z < zEndIdx; z++) {
			for (let y = yStartIdx; y < yEndIdx; y++) {
				for (let x = xStartIdx; x < xEndIdx; x++) {
					...
					const cacheName = is4D
						? `${store}_${idx4D}_${variable}_chunk_${chunkID}`
						: `${store}_${variable}_chunk_${chunkID}`;
					const chunk = cache.get(cacheName);
					const compressed = chunk.compressed;
					const thisData = compressed
						? DecompressArray(chunk.data)
						: chunk.data;
					if (!chunkShape) {
